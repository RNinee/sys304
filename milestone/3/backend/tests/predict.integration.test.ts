...
const INFER_PORT = 19378;
const MODEL = "qwen2.5-1.5b-lora-fp16";
      ...
      async fetch(request) {
        ...
        if (request.method === "GET" && (path === "/health" || path === "/")) {
          return Response.json({
            ...
            role: "infer-worker",
            backend: "fp16",
            optimizations: ["fp16", "merged-lora"],
          });
        }
        ...
        if (request.method === "POST" && path === "/predict_batch") {
          const body = (await request.json()) as {
            items?: Array<{ text?: string; keyword?: string }>;
          };
          const items = body.items ?? [];
          return Response.json({
            results: items.map((item) =>
              classify(item.text ?? "", item.keyword),
            ),
          });
        }
